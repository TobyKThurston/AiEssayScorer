...
export function EssayTextPanel({
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden bg-[#EEF2F7]">
      ...
      {rating && (
        <div className="flex items-center gap-1 px-4 py-2 bg-white border-b border-hair flex-shrink-0 z-10">
      ...
      <div className="flex-1 min-h-0 flex flex-col px-3 sm:px-6 py-4 sm:py-6">
        <div className="flex-1 min-h-0 flex flex-col max-w-[700px] w-full mx-auto">
          ...
          {/* Paper card */}
          <div className="flex-1 min-h-0 flex flex-col bg-white rounded-2xl shadow-[0_4px_24px_rgba(148,163,184,0.18)] border border-hair/60 overflow-hidden">

            {/* Prompt banner (if set) */}
            {essayPrompt && (
              <div className="px-5 sm:px-8 py-3 sm:py-4 bg-[#EFF6FF] border-b border-[#BFDBFE] flex-shrink-0">
                <p className="text-[10px] font-bold text-oxblood uppercase tracking-widest mb-1">
                  Prompt
                </p>
            ...
            {(essayType || targetSchools.length > 0) && (
              <div className="flex items-center gap-2 px-5 sm:px-8 py-2.5 border-b border-slate-100 flex-shrink-0 flex-wrap">
                {essayType && (
                  <span className="px-2.5 py-0.5 rounded-full bg-paper-2 text-ink-2 text-xs font-medium">
                    {essayType}
                  </span>
                )}
                {targetSchools.slice(0, 4).map((s) => (
                  <span
                    key={s}
                    className="px-2.5 py-0.5 rounded-full bg-paper-2 text-ink-2 text-xs"
                  >
                    {s}
                  </span>
                ))}
                {targetSchools.length > 4 && (
                  <span className="text-xs text-pencil">+{targetSchools.length - 4} more</span>
                )}
              </div>
            )}
            ...
            <div className="flex-1 min-h-0 flex flex-col">
              {mode === "edit" ? (
                <textarea
                  className="flex-1 min-h-0 w-full px-5 sm:px-8 py-5 sm:py-7 text-[15px] text-ink bg-transparent outline-none resize-none leading-[1.85] placeholder:text-[#CBD5E1] font-[Georgia,serif]"
            ...
            <div className="px-5 sm:px-8 py-3 border-t border-slate-100 flex items-center justify-between bg-slate-50/60 flex-shrink-0">
              <span className={`text-xs font-medium ${wcColor}`}>
                ...
                {wcStatus === "short" && wordCount > 0 && (
                  <span className="ml-1.5 text-pencil">({presetTarget.min - wordCount} to go)</span>
                )}
                {wcStatus === "good" && wordCount > 0 && (
                  <span className="ml-1.5 text-[#10B981]">(within limit)</span>
                )}
              </span>
              <span className="flex items-center gap-1.5 text-xs text-pencil">
                <span>Limit:</span>
                {editingLimit ? (
                  <input
                    type="number"
                    className="w-16 text-xs text-ink-2 bg-white border border-[#3B82F6] rounded px-1.5 py-0.5 outline-none"
                    value={limitInput}
                  ...
                  <button
                    ...
                    className="text-ink-2 font-medium hover:text-oxblood transition-colors underline decoration-dotted underline-offset-2"
