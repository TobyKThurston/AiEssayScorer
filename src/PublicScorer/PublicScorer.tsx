      ...
      >
        <div>
          ...
          <textarea
            ...
            required
          />
          <div className="text-xs text-[#94A3B8] mt-2">{essay.length}/10,000 chars</div>
        </div>
    ...
    <div className="space-y-4">
      ...
      <div className="rounded-2xl bg-white/70 backdrop-blur-xl border border-white/70 shadow-[0_2px_16px_rgba(99,102,241,0.06)] p-6">
        ...
        <div className="mt-5 pt-5 border-t border-slate-200 flex items-center justify-between">
          <span className="text-sm text-[#475569]">Blend-in risk</span>
          <span
            className={
        ...
        <ul className="space-y-2">
          {rating.strengths.map((s, i) => (
            <li key={i} className="flex gap-2 text-[14.5px] text-[#0F172A] leading-relaxed">
              <Check className="w-4 h-4 text-[#065F46] flex-shrink-0 mt-1" />
              <span>{s}</span>
            </li>
          ))}
        </ul>
      ...
      <div className="rounded-2xl bg-white/70 backdrop-blur-xl border border-white/70 shadow-[0_2px_16px_rgba(99,102,241,0.06)] p-6">
        ...
        <ol className="space-y-3">
          {rating.improvements.map((s, i) => (
            <li key={i} className="flex gap-2 text-[14.5px] text-[#0F172A] leading-relaxed">
              <span className="font-semibold text-[#BE185D]">{i + 1}.</span>
              <span>{s}</span>
            </li>
          ))}
        </ol>
    ...
    <div className="rounded-2xl bg-gradient-to-br from-[#0F172A] to-[#1E293B] p-7 text-white">
      ...
      <div className="flex flex-wrap gap-3">
        <Link
          ...
          className="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl bg-white text-[#0F172A] font-semibold text-sm hover:bg-white/95 transition-colors"
        >
          Sign up free
        </Link>
        <Link
...
function ScoreGate({ variant }: { variant: "login" | "pro" }) {
  return (
    <div className="rounded-2xl bg-gradient-to-br from-[#6366F1] via-[#7C3AED] to-[#8B5CF6] p-8 text-white shadow-[0_8px_32px_rgba(99,102,241,0.3)]">
      <div className="flex items-center gap-2 mb-3">
      ...
      <div className="flex flex-wrap gap-3">
        ...
        <Link
          href="/tools"
          className="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl bg-transparent text-white font-semibold text-sm border border-white/40 hover:bg-white/10 transition-colors"
        >
          Try our free tools
        </Link>
      </div>
