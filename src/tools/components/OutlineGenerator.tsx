  ...
  return (
    <div className="space-y-6">
      ...
      <form onSubmit={handleSubmit} className="rounded-2xl bg-white/60 backdrop-blur-xl border border-white/70 shadow-[0_2px_16px_rgba(99,102,241,0.06)] p-7 space-y-5">
        <div>
          <label className="block text-sm font-semibold text-[#0F172A] mb-2">Essay topic</label>
          <input
            type="text"
            value={topic}
        ...
        <div>
          <label className="block text-sm font-semibold text-[#0F172A] mb-2">
            Word limit: <span className="font-normal text-[#6366F1]">{wordLimit}</span>
          </label>
          <input
            type="range"
            min={100}
      ...
      {result && (
        <div className="space-y-4">
          <Block icon={<Anchor className="w-4 h-4" />} title="Hook" accent="#6D28D9" accentBg="#EDE9FE" words={result.wordAllocation?.hook}>
            <p className="text-[#0F172A] text-[15px] leading-relaxed">{result.hook}</p>
          </Block>
          <Block icon={<Film className="w-4 h-4" />} title="Scene beats" accent="#1D4ED8" accentBg="#DBEAFE" words={result.wordAllocation?.scene}>
            <ol className="space-y-2">
              {result.sceneBeats.map((b, i) => (
                <li key={i} className="flex gap-2 text-[#0F172A] text-[15px] leading-relaxed">
          ...
          <Block icon={<Flag className="w-4 h-4" />} title="Landing line" accent="#BE185D" accentBg="#FCE7F3" words={result.wordAllocation?.landing}>
            <p className="text-[#0F172A] text-[15px] leading-relaxed">{result.landingLine}</p>
          </Block>
  ...
  return (
    <div className="rounded-2xl bg-white/70 backdrop-blur-xl border border-white/70 shadow-[0_2px_16px_rgba(99,102,241,0.06)] p-6">
      <div className="flex items-center justify-between mb-3">
        ...
        {words != null && <span className="text-xs text-[#94A3B8]">~{words} words</span>}
      </div>
      {children}
    </div>
  );
