  ...
  return (
    <div className="space-y-6">
      ...
      <form
        ...
        className="rounded-2xl bg-cream border border-hair shadow-[0_2px_16px_rgba(99,102,241,0.06)] p-7 space-y-4"
      >
        ...
        {activities.map((val, i) => (
          <div key={i}>
            <label className="block text-xs font-semibold text-pencil mb-1">
              Activity {i + 1}
            </label>
            <input
              type="text"
      ...
      {result && (
        <div className="space-y-4">
          ...
          <Panel title="Top strengths" icon={<CheckCircle2 className="w-4 h-4" />} accent="#065F46" accentBg="#D1FAE5">
            <ul className="space-y-2">
              {result.topStrengths.map((s, i) => (
                <li key={i} className="flex gap-2 text-ink text-[14.5px] leading-relaxed">
          ...
          <Panel title="Gaps & overrepresented themes" icon={<AlertCircle className="w-4 h-4" />} accent="#BE185D" accentBg="#FCE7F3">
            ...
            <p className="mt-4 text-[14.5px] text-ink-2 leading-relaxed border-l-2 border-[#FCE7F3] pl-3">
              <span className="font-semibold text-ink">Overlap: </span>{result.overlap}
            </p>
          </Panel>
              ...
              <div className="space-y-3">
                {result.weakest.map((w, i) => (
                  <div key={i} className="rounded-lg bg-cream p-3">
                    <p className="font-semibold text-ink text-sm">{w.label}</p>
                    <p className="text-sm text-pencil mt-1">{w.why}</p>
                    <p className="text-sm text-ink mt-2"><span className="font-semibold">Fix:</span> {w.fix}</p>
                  </div>
                ))}
              </div>
            </Panel>
